...
def plot_radar_data(fig, radar, tilt, dtype, drange, radar_file, shp_path, map_visible=False):
    """
    绘制雷达数据并返回结果
    ...
    :param shp_path: shapefile 路径（可选，用于叠加地图）
    ...
    try:
        # 读取数据
        ds = radar.get_data(tilt=tilt, drange=drange, dtype=dtype)
        data = ds[dtype]
        ...
        cmap = cmap_plot.get(dtype_upper, plt.get_cmap("turbo"))
        norm = norm_plot.get(dtype_upper, None)
        ...
        fig.colorbar(pcm, ax=ax, label=dtype)
