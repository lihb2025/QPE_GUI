...
class RadarViewer(QMainWindow):
    ...
    def init_main_interface(self):
        ...
        if self._scroll_cid is None:
            self._scroll_cid = self.canvas.mpl_connect("scroll_event", self._on_scroll_mpl)
    ...
    def plot_data(self):
        ...
        try:
            ...
            self._orig_extent = tuple(extent)
        except Exception:
            self._orig_extent = None

        gl = self.ax.gridlines(draw_labels=True, linewidth=0.0, color='gray', alpha=0.5)
        gl.top_labels = False
        gl.right_labels = False

        filename = os.path.basename(self.radar_file)
        ...
        # 保存网格数据
        try:
            self._lon = lon.values
            self._lat = lat.values
    ...
    def _on_scroll_mpl(self, event):
        if self.ax is None or event.inaxes != self.ax or event.xdata is None or event.ydata is None:
            return
        try:
            center_lon, center_lat = ccrs.PlateCarree().transform_point(event.xdata, event.ydata, self.ax.projection)
...
# 额外：绑定 Matplotlib 的 scroll_event 到 _on_scroll_mpl（全局只绑一次）
# 需要在 canvas 已创建后绑定，这里放在创建后
def bind_scroll():
    if viewer.canvas is not None:
        # 如果没有绑定过再绑定
        if not hasattr(viewer, "_scroll_cid") or viewer._scroll_cid is None:
            viewer._scroll_cid = viewer.canvas.mpl_connect("scroll_event", viewer._on_scroll_mpl)
# 由于 init_main_interface 在 load_file 后才会创建 canvas，延迟绑定
